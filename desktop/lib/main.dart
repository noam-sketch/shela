...
class _IdeWorkspaceState extends State<IdeWorkspace> with TickerProviderStateMixin {
  ...
  void startTelemetryTimer() {
    _telemetryTimer = Timer.periodic(const Duration(milliseconds: 500), (timer) async {
      try {
        ...
        final file = File(p.join(target, 'logs', 'usage.json'));
        final legacyFile = File(p.join(target, 'logs', '.shela_telemetry.json'));
  ...
  Future<void> _appendToStateFile(String directory, String text) {
    ...
    _stateFileWriteTask = previousTask.then((_) async {
      try {
        final statePath = p.join(directory, 'logs', '.shela_duo_state.md');
  ...
  void runDuo() async {
    ...
    if (session.pty != null) {
      String duoPath = p.join(p.dirname(Platform.resolvedExecutable), 'lib', 'duo.py');
      if (!File(duoPath).existsSync()) {
        // Fallback for development: check workspace core/
        duoPath = p.join(currentDir, 'core', 'duo.py');
      }
      if (!File(duoPath).existsSync()) {
        // Fallback for when running from desktop/lib/ but root is ..
        duoPath = p.join(p.dirname(currentDir), 'core', 'duo.py');
      }
...
''';
  ...
  void _publishWorkspace() async {
    ...
    try {
      ...
      final stateFile = File(p.join(currentDir, 'logs', '.shela_duo_state.md'));
